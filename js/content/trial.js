...
CppLab.content.trial = {
  ...
  activities: [
     ...
     * ========================================================= */
    {
      ...
      variants: {
        E: {
          intro: '看，搭档面前有一条通道：先是空地，再是一节发光的电池，最里面是一扇锁住的门。这里有三块指令积木，把它们拖到轨道上排好顺序，然后按"运行"，搭档就会照着做！',
          program: null,
          interaction: {
            items: [
          ...
          successCriteria: '孩子完成拖拽排序并运行成功；能用自己的话说出"为什么捡电池要在开门前面"。'
        },
        S: {
          intro: '通道更长了！搭档要走到电池旁、捡起电池、再走到门口、用电池的能量开门。这里有四块指令积木，排好顺序再运行。排之前先想一想：如果把"捡电池"放到最后，会发生什么？',
          program: null,
          interaction: {
            items: [
        ...
        A: {
          intro: '高级任务：这次的指令堆里混进了五块积木，其中一块放错位置整个任务就失败。排好顺序、先说出你的预测，再运行验证。运行完试试：故意换一个顺序，猜猜会在第几步卡住？',
          program: null,
          interaction: {
            items: [
     ...
     * program 存带 bug 版；三档 stdout 同为 "门还是关的"
     * ========================================================= */
    {
      ...
      compilerCheck: { enabled: true, expectedStdout: '门还是关的' }
    },
