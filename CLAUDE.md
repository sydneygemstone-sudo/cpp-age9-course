...
- [ ] 三条侧线 + 产品化——**阶段 3 已在 `theater-mode` 分支完成**（2026-08-15 凌晨，Dean 拍板提前开工；分支已推远端）：剧场模式外壳套用真实 app + 第 1 课登机仪式 + 第 2 课文件与工坊 + 双语术语 chip。分支上 7 测试 2033 断言全绿、剧场版 CDP 28/0（脚本 `qa/cdp-lesson1-theater.mjs`）。**main 未动、未合并——等 Dean 首课后过目 theater-mode 分支再拍板合并**（worktree 在项目内 `.worktrees/theater-mode/`，已 gitignore；Dean 规矩：worktree 一律放项目文件内）。方向定案见 `docs/调研-三侧线与产品化-20260814.md`。
